...
interface Team {
  _id: string;
  name: string;
  owner?: string; // Assuming you are adding this field
  logo: string;
  pointsUsed: number;
}
  ...
  return (
    <div className="flex h-[calc(100vh-64px)] bg-[#050505] text-white overflow-hidden font-sans">
      ...
      <main className="flex-1 flex flex-col items-center justify-between py-4 px-6 relative">
        ...
        {/* BOTTOM TEAM DASHBOARD - Shows Team Name and Owner Name */}
        <div className="grid grid-cols-6 gap-3 w-full max-w-6xl pb-2">
          {teams.map(team => (
            <Link href={`/team/${encodeURIComponent(team.name)}`} key={team._id}>
              <div className="bg-slate-900/40 border border-slate-800 p-3 rounded-2xl text-center cursor-pointer transition-all hover:border-blue-500 active:scale-95 flex flex-col justify-between min-h-[120px]">
                
                {/* Team Name and Owner Display */}
                <div className="flex-1 flex flex-col items-center justify-center gap-1">
                  <p className="text-lg font-black text-white uppercase leading-none tracking-tight">
                    {team.name}
                  </p>
                  <p className="text-[15px] font-bold text-blue-400 uppercase tracking-widest opacity-80">
                    {team.owner || "Owner Name"}
                  </p>
                </div>
                
                {/* Points Status */}
                <div className="bg-black/50 rounded-lg py-2 border border-slate-800">
                  <p className="text-2xl font-mono font-bold text-emerald-400 leading-none">
                    {100 - (team.pointsUsed || 0)}
                  </p>
                  <p className="text-[8px] font-black text-slate-500 uppercase tracking-widest mt-1">Points Left</p>
                </div>
              </div>
            </Link>
          ))}
